...
import { Container, LaunchTitle, MountainsImage, Page, SocialIcon, SocialIconContainer, TimerContainer } from "./CountdownTimer.styled";

import starsBg from "./img/bg-stars.svg";
import mountainsImage from "./img/pattern-hills.svg";
import TimerElement from "./TimerElement";


function CountdownTimer(){
    const [time,setTime] = React.useState(777341);
    ...
    return(
        <Page>
            <Container backgroundImage={starsBg}>
                <LaunchTitle>WE'RE LAUNCHING SOON</LaunchTitle>
                <TimerContainer>
                    <TimerElement value={Math.floor(time/60/60/24)} title="DAYS" />
                    <TimerElement value={Math.floor(time/60/60%24)} title="HOURS" />
                    <TimerElement value={Math.floor(time/60%60)} title="MINUTES" />
                    <TimerElement value={Math.floor(time%60)} title="SECONDS" />
                </TimerContainer>
                <MountainsImage src={mountainsImage} alt="mountains" />
                <SocialIconContainer>
